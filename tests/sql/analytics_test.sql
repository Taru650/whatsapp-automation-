...
INSERT INTO core.message_log (wa_msg_id, wa_hash, direction, service_key, subtype, kind, text, via, resolved, llm_tokens, latency_ms, detail, error, ts) VALUES
    ...
    ('m4', 'c2bbbbbbbbbbbbbbbbbbb', 'in', 'mela', 'today', 'list', NULL, 'list', true, NULL, 300, NULL, NULL, '2026-08-01 23:30+05:30'),
...
BEGIN
    ...
    ASSERT d.citizens = 2 AND d.turns = 4, format('citizens/turns %s/%s', d.citizens, d.turns);
    ASSERT d.new_citizens = 2, 'new citizens (admin excluded)';
    ASSERT d.menu_taps = 2 AND d.typed = 1 AND d.locations = 1, 'channel split';
    ASSERT d.resolved_pct = 75.0, format('resolved_pct %s', d.resolved_pct);
    ASSERT d.unanswered = 1 AND d.thumbs_down = 1 AND d.thumbs_up = 0, 'admin feedback/unanswered excluded';
    ASSERT d.send_errors = 1 AND d.llm_tokens = 1000 AND d.via_llm = 1, 'errors and tokens';
    ...
    ASSERT (r->>'ok')::boolean AND (r->>'raw_inbound')::int = 6 AND (r->>'admin_inbound')::int = 2, r::text;
    ...
    ASSERT (SELECT count(*) FROM analytics.v_topics WHERE day = '2026-08-01') = 4, 'topics';
    ...
    FOR col IN SELECT table_name, column_name FROM information_schema.columns
        ...
        EXECUTE format('SELECT count(*) FROM analytics.%I WHERE %I ~ ''\d{6,}|\d{3,}[ -]\d{3,}''', col.table_name, col.column_name) INTO n;
    ...
    ASSERT r->>'text' LIKE '%Citizens: 2 (2 new) · messages: 4%', r->>'text';
    ...
    ASSERT (SELECT (stats->>'turns')::int FROM analytics.daily_archive WHERE day = '2026-08-01') = 4, 'archived';
